      ...
      async loader(getAuth, provider) {
        ...
        return {
          ...
          async fetch(request: RequestInfo | URL, init?: RequestInit) {
            ...
            // Token exchange logic: Exchange GitHub OAuth token for Copilot API token
            if (!info.access || (info.expires || 0) < Date.now()) {
              const domain = info.enterpriseUrl ? normalizeDomain(info.enterpriseUrl) : "github.com"
              const urls = getUrls(domain)

              // Fetch Copilot API Token using the OAuth Refresh Token (which behaves as the access token here)
              ...
              await client.auth.set({
                ...
                body: {
                  type: "oauth",
                  refresh: info.refresh, // Keep the original OAuth token
                  access: tokenData.token, // Store the Copilot API token as 'access'
            ...
            const headers: Record<string, string> = {
              ...
              ...HEADERS,
              Authorization: `Bearer ${info.access}`, // Use the Copilot API token
                ...
                while (true) {
                  ...
                  if (data.access_token) {
                    const result: {
                      ...
                      access: "", // Empty - will be exchanged for Copilot API token in loader
                      expires: 0,
                    }
